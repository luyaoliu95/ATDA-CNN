...
def get_data(wav_files, fixed_t, silence_delete, feature_to_use, label_file, file_type):
    ...
    return mfccs, delta, delta, emo_labels, sex_labels
...
def process_MELD(root_path, seed=987654, fixed_t=6, silence_delete=True, feature_to_use='mfcc', test_cluster=5):
    ...
    train_mfcc, train_delta, train_delta_delta, train_emo, train_sex = get_data(train_wav_files, fixed_t, silence_delete, feature_to_use, train_label_file, 'train')
    dev_mfcc, dev_delta, dev_delta_delta, dev_emo, dev_sex = get_data(dev_wav_files, fixed_t, silence_delete, feature_to_use, dev_label_file, 'dev')
    valid_mfcc, valid_delta, valid_delta_delta, valid_emo, valid_sex = get_data(test_wav_files, fixed_t, silence_delete, feature_to_use, test_label_file, 'test')

    all_mfcc = np.concatenate((train_mfcc, dev_mfcc, valid_mfcc), axis=0)
    all_delta = np.concatenate((train_delta, dev_delta, valid_delta), axis=0)
    all_delta_delta = np.concatenate((train_delta_delta, dev_delta_delta, valid_delta_delta), axis=0)
    ...
    for n in range(test_cluster):
        ...
        for i in train_indices:
            train_mfcc.append(all_mfcc[i])
            train_delta.append(all_delta[i])
            train_delta_delta.append(all_delta_delta[i])
            train_emo.append(all_emo[i])
            train_sex.append(all_sex[i])

        for i in valid_indices:
            valid_mfcc.append(all_mfcc[i])
            valid_delta.append(all_delta[i])
            valid_delta_delta.append(all_delta_delta[i])
        ...
        save_path = '.\data\MELD_data_{}.pkl'.format(n)
...
if __name__ == '__main__':
    ...
    process_MELD(root_dir, seed=987654, fixed_t=6, silence_delete=True, feature_to_use='mfcc')
