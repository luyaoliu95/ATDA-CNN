...
def process_IEMOCAP(path, seed=987654, fixed_t=6, test_cluster=1, split_method='random', silence_delete=True, data_aug=False, feature_to_use='mfcc'):
    ...
    for n in range(test_cluster):
        ...
        train_mfcc, train_delta, train_delta_delta, train_emo, train_sex = get_data(files=train_data_dict[str(n+1)], fixed_t=fixed_t, silence_delete=silence_delete, feature_to_use=feature_to_use)
        valid_mfcc, valid_delta, valid_delta_delta, valid_emo, valid_sex = get_data(files=valid_data_dict[str(n+1)], fixed_t=fixed_t, silence_delete=silence_delete, feature_to_use=feature_to_use)
