...
class TCANet(nn.Module):
    ...
    def __init__(self, emb_size, input_output_size, num_channels, seq_len, num_sub_blocks, temp_attn, nheads, en_res,
        ...
        self.Linear = nn.Linear(in_features=217, out_features=emb_size)
    ...
    def forward(self, input):
        ...
        # emb = self.drop(self.word_encoder(input))
        batch_size, channel, dim = input.shape[0], input.shape[1], input.shape[2]
        emb = self.Linear(torch.reshape(input, (batch_size*channel, dim))).view(batch_size, channel, -1)
        if self.temp_attn:
