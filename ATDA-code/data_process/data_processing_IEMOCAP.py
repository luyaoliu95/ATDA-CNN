...
def process_IEMOCAP(path, seed=987654, fixed_t=6, test_cluster=1, split_method='random', silence_delete=True, data_aug=False, feature_to_use='mfcc'):
    ...
    for n in range(test_cluster):
        ...
        train_x, train_emo, train_sex = get_data(files=train_data_dict[str(n+1)], fixed_t=fixed_t, silence_delete=silence_delete, feature_to_use=feature_to_use)
        valid_x, valid_emo, valid_sex = get_data(files=valid_data_dict[str(n+1)], fixed_t=fixed_t, silence_delete=silence_delete, feature_to_use=feature_to_use)
        ...
        save_path = '..\data\without_VAD\\IEMOCAP_leave_' + str(n+1) + '_data_speaker.pkl'
...
if __name__ == '__main__':
    ...
    process_IEMOCAP(r'K:\IEMOCAP', test_cluster=10, split_method='speaker', silence_delete=False, data_aug=False)
